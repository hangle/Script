...
case class LoadAssignCmd(parameters:List[String]) extends Node with Link  with Common {
	override def toString="LoadAssignCmd"
			// LoadAssignCmd is in a linked list whose parent is LoadDictionaryCmd
			// The parent created this list and in doing so, it 
			// recorded in LoadAssignCmd (via Node.next) the reference 
